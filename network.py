...
class Network(NetworkGenerator):
    def __init__(self,network_type=None,network_spec=None,
        ...
        Initialises some class properies.
    ...
    def initialise_network(self,node_spec,input_amp=None,output_amp=None):
        ...
        ----------
        scat_mat_type : str
            See NODES.init_Smat docstring for further details
        scat_loss : float
        ...
        Smat_params : dict, optional
            Dictionary of Keyword arguments required for specified scattering matrix type.
            The default is None.
        ...
        **kwargs : Keyword arguments
        ...
        if node.scat_mat_type == 'identity':
            ...
            if mat.shape  != (node.n_connect,node.n_connect):
                raise RuntimeError("Supplied scattering matrix is of incorrect dimensions: {} supplied, {} expected".format(mat.shape,(node.n_connect,node.n_connect)))
            else:
                node.S_mat = mat
        elif node.scat_mat_type == 'unitary_cyclic':
            if 'delta' in kwargs.keys(): 
                l = np.exp(1j*kwargs['delta'][0:node.n_connect])
            else:
                l = np.exp(1j*2*np.pi*np.random.rand(node.n_connect))
            s = np.matmul((1/node.n_connect)*dft(node.n_connect),l)
    ...
    def run_network(self,
                    ...
                    conv_nodes='all',
                    direction='forward'):
        output = []            #the list modulus squared waves leaving the system at each iteration
        scat_loss_output = []
        total_var = 1
        ...
        if converge == False:
            for niter in range(1,n_iterations+1):
                ...
                # # store total output power from each iteration
                # output_power = 0
                # for i in range(0,self.total_nodes):
                #     node = self.get_node(i)

                #     if node.node_type == 'exit':
                #         for x in node.inwave:
                #             output_power += abs(node.inwave[x])**2 if direction == 'forward' else abs(node.outwave[x])**2 #modulus squared of output
                # output.append(output_power)
            ...
            for niter in range(1,n_iterations):
                update_progress(threshold/abs(total_var - threshold),'Iteration {}/{} : variance {}'.format(niter,n_iterations,total_var))
                self.update_network(direction)    # updates and iterate
                ...
                if niter % period == 0:
                    # print('***********************************')
                    # print([sum(stats.tstd(np.abs(coeffs))) , sum(np.mean(np.abs(coeffs),axis=0))])
                ...
                #         if node.node_type == 'exit':
                #             for x in node.inwave:
                #                 output_power += abs(node.inwave[x])**2 if direction == 'forward' else abs(node.outwave[x])**2

                #     output.append(output_power)
                #     accuracy = (sum(abs(i)**2 for i in self.input)*percentage) if direction == 'forward' else (sum(abs(i)**2 for i in self.output)*percentage)

                #     if niter % period == 0:
                #         check_for_convergence = output[-period:]
                #         try:
                #             if stats.tstd(check_for_convergence) < accuracy:
                #                 print ('Converged after ',niter,' iterations')
                #                 break
                #             else:
                #                 pass
                ...
                # elif self.scat_loss != 0:
                #     for i in range(0,self.total_nodes):
                #         node = self.get_node(i)
                #         if node.node_type == 'internal':
                #             for u in node.sorted_connected_nodes:
                #                 scattered_power += abs(node.outwave['loss%u'%u])**2
                #         elif node.node_type == 'exit':
                #             for x in node.inwave:
                #                 output_power += abs(node.inwave[x])**2 if direction == 'forward' else abs(node.outwave[x])**2
                #     scat_loss_output.append(scattered_power)
                #     output.append(output_power)

                #     accuracy = (sum(abs(i)**2 for i in self.input)*percentage) if direction == 'forward' else (sum(abs(i)**2 for i in self.output)*percentage)
                #     if niter % period == 0:
                #         check_for_convergence = [a + b for a, b in zip(output[-period:], scat_loss_output[-period:])]
                #         try:
                #             if stats.tstd(check_for_convergence) < accuracy:
                #                 print ('Converged after ',niter,' iterations')
                #                 break
    ...
    def find_pole(self,k0,method='CG',opts=None,bounds=None):
        ...
        optout = optimize.minimize(lambda kk:self._find_pole_helper(kk[0],kk[1],opt_network),
                                    [np.real(k0),np.imag(k0)],method=method,options=opts,bounds=bounds)
    ...
    def calc_input_energy(self,Z=1.0):
        '''
        Calcualtes the total power input into the network based on exit node amplitudes.
        ...
        for node in self.nodes:
            if node.node_type == 'exit':
                input_power += np.sum(np.abs(np.array(list(node.outwave.values())))**2) # note energy going into network is going out from the exit node

        return input_power / Z

    def calc_output_energy(self,Z=1.0):
        '''
        ...
        return output_power / Z


    def calc_internal_energy(self,epsr=1):
        '''
        Calcualtes the total energy density within the network.
        ...
        for connection in self.links:
            # if connection.link_type == 'internal':
            ...
            if ki == 0:
                ...
                # UintPP2 = Ap * Apc * 1j * d**2 / 2
                # UintMM2 = Am * Amc * 1j * d**2 / 2
                # UintPM2 = Ap * Amc * (kr*d*np.exp(1j*d*kr) - np.sin(kr*d) ) / (2*kr**2)
                # UintMP2 = Am * Apc * (kr*d*np.exp(1j*d*kr) - np.sin(kr*d) ) / (2*kr**2)
            else:
                ...
                # UintPP2 = Ap * Apc * 1j * np.exp(-2*d*ki) * (np.exp(2*d*ki) - 1 - 2*d*ki) / (4*ki**2)
                # UintMM2 = Am * Amc * 1j * np.exp(-2*d*ki) * (np.exp(2*d*ki) - 1 - 2*d*ki) / (4*ki**2)
                # UintPM2 = Ap * Amc * np.exp(-d*ki) * (kr*d*np.exp(1j*d*kr) - np.sin(kr*d) ) / (2*kr**2)
                # UintMP2 = Am * Apc * np.exp(-d*ki) * (kr*d*np.exp(1j*d*kr) - np.sin(kr*d) ) / (2*kr**2)

            U1 = 2*0.5*eps0*epsr*(UintPP  + UintMM  + UintPM  + UintMP ) # twice since we assume Ue = Um
            U2 = 0#2*0.5*eps0*epsr*(UintPP2 + UintMM2 + UintPM2 + UintMP2) # twice since we assume Ue = Um
    ...
    def connected_component_nodes(self,):
        ...
        components : list of list
            lists of node ids present in each consituent network component
...
######################################################
    ...
    def scattering_matrix_recursive(self,n_iterations=1000,converge=True,period=200,threshold=0.01):
        '''
        Calculates the scattering matrix of the network using a forward recursive solution of the matrix transport equations
    ...
    def inverse_scattering_matrix_recursive(self,n_iterations=1000,converge=True,period=200,threshold=0.01):
        '''
        Calculates the inverse scattering matrix of the network using a backward recursive solution of the matrix transport equations
    ...
    def draw(self,draw_mode='',fig=None):
        ...
        if draw_mode == 'intensity':
            ...
            for connection in self.links:
                ...
                d = np.array([self.calculate_distance(node1.position,(x[i],y[i])) for i in range(0,Np)]) # avoid end points and thus repeated points
                # calculate intensity along link
                field = connection.inwave[0] * np.exp(1j*self.k*d) + connection.inwave[1] * np.exp( - 1j*self.k*(connection.distance - d))
                ...
                if node.node_type == 'internal':
                    ...
                    for i in node.outwave:
                        if node.outwave[i] == 0 :
                            ...
                            # plt.text(node.position[0], node.position[1],"EXIT",bbox=dict(facecolor='red',alpha=0.5), size =15 ,color='white')
                        else :
                            plt.plot(node.position[0], node.position[1], 'o',color='red',markersize=ms)
                            # plt.text(node.position[0], node.position[1],"INJECTION",bbox=dict(facecolor='green',alpha=0.5), size =15 ,color='white')
            ...
            for node in self.nodes:
                ...
                if node.node_type == 'internal':
                    ...
                    # plt.text(node.position[0], node.position[1],str(node.number),bbox=dict(facecolor='blue',alpha=0.5), size =15 ,color='white')

                elif node.node_type == 'exit':
                    plt.plot(node.position[0], node.position[1], 'o',color='#85C27F')
                    # plt.text(node.position[0], node.position[1],str(node.number),bbox=dict(facecolor='red',alpha=0.5), size =15 ,color='white')
