...
class NetworkGenerator:
    ...
    def generate_delaunay(self,spec):
        ...
        if spec['shape'] == 'circular':
            ...
            rint = network_size*np.sqrt(np.random.random(self.internal_nodes-self.exit_nodes)) # square root gives a more uniform distribution of points
    ...
    def generate_voronoi_circular(self,spec):
        ...
        r = network_size*np.sqrt(np.random.random(seed_nodes)) # square root gives a more uniform distribution of points
        points = np.array([r*np.cos(t),r*np.sin(t)]).T
        if exit_size <= network_size:
            raise ValueError('exit_size must be larger than network_size')
        ...
        vor_ridge_points = [vor_ridge_points[num] for num,ridge in enumerate(vor_ridges) if num not in ridge_inds_to_delete]
    ...
    def generate_voronoi_slab(self,spec):
        ...
        while not correct_exits:
            ...
            for ii in range(0,len(self.links)):
                ...
                if (int_ptU.is_empty == False) and (int_ptL.is_empty == False): # intersect with upper and lower boundary
                    initnode1 = connection1.node1
                    initnode2 = connection1.node2
                ...
                elif (int_ptU.is_empty == False): # intersect with upper boundary
                    ...
                    self.add_connection(intersect_node_id,initnode,self.calculate_distance(A, int_pos),self.k,self.n,'internal')
                    intersectionsU[intersect_node_id] = {'ridge':ii,
                                                        'position':int_pos,
                                                        'node1':intersect_node_id,
                                                        'node2':initnode,
                                                        }
                elif (int_ptL.is_empty == False): # intersect with lower boundary
                    # get id for node within bounding rectangle
                    if (abs(Ax) <= xb) and (abs(Ay) <= yb) :
                        initnode = connection1.node1
                    else:
        ...
        while (self.exit_nodes != external_link_number) :
            ...
            for nn in range(0,number_of_lines):
                if spec['shape'] == 'circular':
                    ...
                    points = np.array([network_size*np.cos(t),network_size*np.sin(t)]).T
                elif spec['shape'] == 'slab':
            ...
            for ii in range(0,len(fibres)):
                ...
                for jj in range(ii+1,len(fibres)):
                    ...
                    if (int_pt.is_empty == False): # lines intersect
            ...
            for ii in range(0,len(fibres)):
                ...
                nodes = [inter for inter in intersections if ((intersections[inter]['line1'] == ii)  or (intersections[inter]['line2'] == ii))]
                ...
                # form connections
                for ii in range(0,len(orderednodes)-1):
                    distance = self.calculate_distance(self.get_node(orderednodes[ii]).position,self.get_node(orderednodes[ii+1]).position)
                    self.add_connection(orderednodes[ii], orderednodes[ii+1], distance,self.k,self.n)
...
##########################################################################
                ...
                if node.number == connection.node1 or node.number == connection.node2:
                    ...
                    for othernode in empty:
                        if othernode != node.number:
                            connected_nodes.append(othernode) # create a list of the nodes that this node connected with
    ...
    def trim_network_exits(self,nexit=1,ids=None):
        '''
    ...
    def generate_tiling(self,params):
        ...
        if params['type'] == 'square':
            ...
            edge_nodes = [node for node in self.nodes if (np.abs(np.linalg.norm(node.position) - network_size) <= septol) ]
            ...
            for ix in range(0,len(centerx)): # loop over elements
                # vertex coordinates within elements
                xv = scale*np.array([centerx[ix] + np.sqrt(3)/3 * np.cos(i * 2*np.pi / 3 +  np.pi / 2) for i in range(0,3)] )
                yv = scale*np.array([centery[ix] + np.sqrt(3)/3 * np.sin(i * 2*np.pi / 3 +  np.pi / 2) for i in range(0,3)] )
                # xv =[ centerx[ix]]
                # yv =[ centery[ix]]
                #       0
                #      / \
            ...
            exit_size = self.network_size + scale
            self.total_nodes = self.internal_nodes + self.exit_nodes
            ...
            for ix in range(0,len(centerx)): # loop over elements
                # vertex coordinates within elements
                xv = scale*np.array([centerx[ix] + 1/(2*np.cos(np.pi / 6)) * np.cos(i * np.pi / 3 +  np.pi / 6) for i in range(0,6)] )
                yv = scale*np.array([centery[ix] + 1/(2*np.cos(np.pi / 6)) * np.sin(i * np.pi / 3 +  np.pi / 6) for i in range(0,6)])
                #      /1\
                #    /     \
                #   2       0
                #   |       |
