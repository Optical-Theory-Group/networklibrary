...
class NODE:
    def __init__(self, number: int = None, pos: tuple[float, float] = None, nodetype: str = 'internal',
        ...
        self.sorted_connected_nodes: list[int]  # list of ids of nodes that are connected. order matches that of S_mat
        self.Smat_type: Union[None, str]  # string identifier for nature of scattering matrix
        self.scat_loss: float  # parameter describing fractional scattering loss
        self.Smat_params: Union[None, dict]  # dictionary of any additional parameters used to generate S_mat
        self.inwave: Union[None, dict[int, Union[float, complex]]]  # dictionary of inwave amplitudes
        #                                                               {node id: amplitude, ... }
        self.outwave: Union[None, dict[Union[str, int], Union[float, complex]]]  # dictionary of outwave amplitudes
        #                                           {node id: amplitude, ..., 'loss node id: loss amplitude, ...}
        self.inwave_np: Union[None, np.ndarray[np.complex64]]  # array of inwave amplitudes
        self.outwave_np: Union[None, np.ndarray[np.complex64]]  # array of outwave amplitudes
        self.S_mat: Union[None, np.ndarray[np.complex64]]  # numpy array specifying scattering matrix
        self.iS_mat: Union[None, np.ndarray[np.complex64]]  # numpy array specifying inverse scattering matrix
        ...
        if nodedict is not None:
            ...
            if number is None:
                raise TypeError("Node ID number has not been supplied, but is required")
            if pos is None:
                raise TypeError("Node position has not been supplied, but is required")

            logging.info("...from input arguments: {} node #{} @ {}.".format(nodetype, number, pos))
    ...
    def __setattr__(self, name, value):
        ...
        if name == 'node_type':
            if value not in ['internal','exit']:
                raise AttributeError("Tried to set an invalid value for the node_type attribute")
        if name == 'scat_loss':
            if value < 0 or value > 1:
                raise AttributeError("Tried to set an invalid value for the scat_loss attribute. Most lie between 0 and 1")
    ...
    def update(self, scat_loss: float = 0, direction: str = 'forward') -> None:
        ...
        if direction == 'forward':
            ...
            raise (ValueError, 'Unknown run direction type: must be "forward" or "backward"')
    ...
    def get_default_properties():
        default_values = {'number': None,  # unique numerical id for node
