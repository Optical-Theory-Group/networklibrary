"""
DORT fault-localization performance study -- POINT (node) faults.
...
Run:   nohup python benchmark_dort_node.py > dort_node_study.log 2>&1 &
Smoke: python benchmark_dort_node.py --smoke
"""
...
N_FREQ = 500
FAULT_REFLECTIVITY = 0.1            # point-fault (partial reflector) reflectivity
N_WORKERS = 60
...
STUDY_TAG = f'dort_node_r{FAULT_REFLECTIVITY}_{METHOD}'
...
def reflector_node(r):
    """Lossless partial-reflector S-matrix, its inverse, and dS/dx (zero)."""
    t = 1.0 - r
    S = np.array([[np.sqrt(r), np.sqrt(t)],
                  [np.sqrt(t), -np.sqrt(r)]], complex)
    return (lambda k: S, lambda k: S, lambda k: np.zeros((2, 2), complex))
...
def process_fault(task):
    ...
    try:
        # ---- place a point (node) fault -> faulty S(k), same k order as the precompute ----
        S_f, S_fi, dS_f = reflector_node(FAULT_REFLECTIVITY)
        pb = NetworkPerturbator(net)
        pb.add_perturbation_node(
            link_index=link_idx, fractional_position=x_f,
            scattering_matrix=S_f, scattering_matrix_inverse=S_fi,
            scattering_matrix_derivative=dS_f)
...
def run_seed(ni, seed, out_dir, n_faults, n_workers):
    ...
    save_results_atomic(out_path, {
        ...
        'config': {'NE': N_EXTERNAL, 'fault_reflectivity': FAULT_REFLECTIVITY,
...
def main():
    print("=== DORT POINT-FAULT Localization Study ===")
