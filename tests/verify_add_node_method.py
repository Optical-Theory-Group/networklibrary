...
""""
We start with the below network and will add the other two nodes (top half of the triangle) (one external node and one internal node)
...
node_dict2 = {
    0: Node(0, "internal", (-1,0), data = data),
    1: Node(1, 'internal', (1,0), data = data),
    2: Node(2, 'external', (-2,0), data = data),
    3: Node(3, "external", (2,0), data = data)
}

link_dict2 = {
    0: Link(0, "internal", (0,1)),
    2: Link(2, "external", (0,2)),
    3: Link(3, "external", (1,3))
    }
...
# Now lets add a node to network2
network2.add_node((0,1), node_connections=[0,1], node_type="internal")
network2.add_node((0,2), node_connections=[2], node_type="external")

# Both networks are essentially the same. and so should produce the same final scattering matrix
...
# plt.savefig("added_node_network.png")

# Test scattering matrix calculation
see1 = network.get_S_ee(k)
see2 = network2.get_S_ee(k)

# Check if all the elements of the two matrices are same
# USE S_EE matrix as a meausre if it is correct because S_EE involves many subcomponents
# it's calculation, So if something goes wrong. highly unlikely they will match
if np.all(see1 == see2):
