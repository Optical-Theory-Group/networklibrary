...
class NetworkEnsemble:
    ...
    def __init__(
        ...
        with h5py.File(savetofilename, "a", driver=None) as h5file:
            ...
            for b in range(0, nbatch):
                if nbatchsize > 1:
                    update_progress(
                        progresscount / totalrealisations,
                        "{}/{}".format(progresscount, totalrealisations),
                    )
                    pool = mp.Pool(mp.cpu_count())
                    seedstorun = [
                        initial_seed + b * nbatchsize + bb
                        for bb in range(0, nbatchsize)
                        if initial_seed + b * nbatchsize + bb
                        not in existing_realisations
                    ]
                    skipped = nbatchsize - len(seedstorun)
                    progresscount += skipped
                    if skipped > 0:
                        print(
                            "\n Skipping {} realisations that already exist.".format(
                                skipped
                            )
                        )
                    poolresult = [
                        pool.apply_async(
                            self.single_run,
                            args=(seed, network_type, network_spec, node_spec),
                            callback=self.collect_result,
                        )
                        for seed in seedstorun
                    ]
                    pool.close()
                    pool.join()
                else:
                    for bb in range(0, nbatchsize):
                        ...
                        if (initial_seed + i) not in existing_realisations:
                            networkdictresult = self.single_run(
                                initial_seed + i,
                                network_type,
                                network_spec,
                                node_spec,
                            ...
                            progresscount += 1
                            print(
                                "\n Realisation {} already exists - skipping".format(
                                    initial_seed + i
                                )
                            )
    ...
    def save_results(self, h5file, writemode):
        ...
        for seednum, networkdictresult in resultstosave.items():
            if writemode == "sm":
                towrite = {
                    "NETWORK": {
                        "scattering_matrix": networkdictresult["NETWORK"][
                            "scattering_matrix"
                        ],
                        "sm_node_order": networkdictresult["NETWORK"][
                            "sm_node_order"
                        ],
                        "exit_ids": networkdictresult["NETWORK"]["exit_ids"],
                        "exit_positions": networkdictresult["NETWORK"][
                            "exit_positions"
                        ],
                    }
                }
            ...
            towrite_all.update(
                {
                    "{}".format(
                        networkdictresult["NETWORK"]["seed_number"]
                    ): towrite
                }
            )
    ...
    def collect_result(result):
        ...
        progresscount += 1
        remaintime = (runtime / progresscount) * (
            totalrealisations - progresscount
        )

        update_progress(
            progresscount / totalrealisations,
            "{}/{}. \n Run time {}. \n Est. time remaining  {}. \n".format(
                progresscount, totalrealisations, runtime, remaintime
            ),
        )
        resultstosave.update(
            {"{}".format(result["NETWORK"]["seed_number"]): result}
        )
