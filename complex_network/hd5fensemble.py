...
def generator_worker(
    queue: "mp.Manager.Queue", i: Any, generator: callable, **genargs
) -> dict:
    """
        ...
        while True:
            if not q.empty():
                ...
                recursively_save_dict_contents_to_group(
                    h5file, "/", processed_data
                )


def remove_existing(
    ivals: Union[Iterator, Generator, list],
    filename: str,
    verbose: Union[int, float, bool] = 1,
) -> (Generator, int):
    """
...
def report_progress(
    ...
    if verbose:
        ...
        runtimesecs = (
            runtime.total_seconds() if runtime.total_seconds() > 0 else 0.1
        )  #
        remaintime = (runtime / progresscount) * (
            totalrealisations - progresscount
        )

        strmsg = "{}/{}" "   in   : {} ({}/s  eta: {}).".format(
            progresscount,
            totalrealisations,
            runtime,
            progresscount / runtimesecs,
            remaintime,
        )
        if verbose > 1:
            if os.path.exists(filename):
            ...
            strmsg = (
                strmsg
                + "    Data  : {:.3f} MB. ({:.3f} kB/s  est.:  {:.3f} MB)  ".format(
                    currentsize,
...
def update_progress(progress: float, status: str = "", barlength: int = 20):
    ...
    text = prelim + "\rPercent: [{0}] {1:.2f}% {2}".format(
...
def generate_ensemble(
    filename: str,
    ivals0: Union[list, Iterator, Generator],
    generator: callable,
    ...
    ivals, skipped = (
        remove_existing(ivals0, filename, verbose=verbose)
        if writemode == "a"
        else (ivals0, 0)
    )
    ...
    with CloudpickleProcessPoolExecutor() as pool:
        with mp.Manager() as manager:
            ...
            computer = [
                pool.submit(
                    generator_worker, q, i, generator, **generator_args
                )
                for i in ivals
            ]

            for cfuture in cf.as_completed(computer):
                progresscount = report_progress(
                    progresscount,
                    totalrealisations,
                    starttime,
                    filename,
